...
export default function TripChat({ tripId }: TripChatProps) {
  ...
  return (
    <div className="border border-gold-light/40 bg-white rounded-xl shadow-sm flex flex-col overflow-hidden">
      {/* Chat header */}
      <button
        className="bg-navy rounded-t-xl px-4 py-2.5 flex items-center justify-between w-full focus:outline-none focus:ring-2 focus:ring-gold/30 focus:ring-inset"
        onClick={() => setCollapsed(!collapsed)}
        aria-expanded={!collapsed}
      >
        <div className="flex items-center gap-2">
          <span className="w-6 h-6 bg-cream rounded-md flex items-center justify-center flex-shrink-0">
            <MessageCircle className="h-3.5 w-3.5 text-navy" aria-hidden="true" />
          </span>
      ...
      {!collapsed && (
        <>
          {/* Messages area */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 max-h-80 min-h-[200px] bg-cream" aria-live="polite" aria-label="Chat messages">
            {messages.length === 0 && (
          ...
          <div className="px-4 py-2 flex gap-2 flex-wrap border-t border-gold-light/30">
            {quickActions.map((action) => (
              <button
                ...
                className="px-2.5 py-1 border border-gold rounded-md font-sans text-[10px] font-medium text-gold hover:bg-gold/8 btn-transition focus:outline-none focus:ring-2 focus:ring-gold/30"
              >
                {action}
              </button>
            ))}
          ...
          <div className="p-3 border-t border-gold-light/30 flex gap-2">
            ...
            />
            <Button variant="primary" size="sm" onClick={handleSend} disabled={!input.trim() || sending} className="flex items-center gap-1.5">
