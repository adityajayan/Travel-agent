...
class OrchestratorAgent:
    ...
    async def _identify_missing_info(
        ...
        prompt = (
            ...
            "- Cheapest flights and best-value accommodations\n"
            "- Off-peak / shoulder season travel for lower prices and fewer crowds\n"
            "- Seasonally optimal activities (best time of year for the destination)\n"
            "- Deals, specials, and promotional rates\n\n"
            "Only ask questions where the customer's answer would MEANINGFULLY change "
            "the trip. Frame questions as optional preferences, not requirements.\n\n"
            "DO NOT ask about:\n"
            "- Budget (we'll optimize for cheapest by default)\n"
            "- Flight class (we default to economy)\n"
            "- Hotel star rating (we find best value)\n"
            "- Things already specified in the request\n\n"
            "DO consider asking about (only if truly unspecified):\n"
            "- Departure city (we need this to book flights)\n"
            "- Approximate travel dates or flexibility (so we can find the best deals)\n"
            "- Number of travelers\n"
            "- Any strong deal-breakers (allergies, accessibility needs, etc.)\n\n"
            "Be very selective — ask at most 2-3 questions. If the request has a "
            "destination and we can reasonably infer the rest, return an empty list.\n\n"
            "Return a JSON array of questions. Each question should have:\n"
            '- "key": a snake_case identifier (e.g. "travel_dates", "departure_city")\n'
            '- "question": the question to ask the user (frame as optional)\n'
    ...
    async def _decompose(self, goal: str) -> dict:
        ...
        prompt = (
            ...
            "IMPORTANT — Cost & value optimization:\n"
            "For each sub-goal, embed optimization hints that guide the specialist agent:\n"
            "- If the customer did NOT specify dates, add 'Choose the cheapest available "
            "dates, preferring shoulder/off-peak season for the destination' to the sub-goal.\n"
            "- If the customer did NOT specify an airline or cabin class, add "
            "'Find the cheapest economy flight option' to the sub-goal.\n"
            "- If the customer did NOT specify hotel preferences, add "
            "'Find the best-value accommodation with good ratings' to the sub-goal.\n"
            "- If the customer did NOT specify activity preferences, add "
            "'Prioritize free or low-cost activities that are seasonally optimal "
            "for the destination' to the sub-goal.\n"
    ...
    async def _synthesize(self, state: TripState) -> str:
        ...
        prompt = (
            ...
            "Include:\n"
            "- A brief overview of what was booked\n"
            "- Any cost savings, deals, or specials that were found\n"
            "- Why this timing/season is a good choice (if relevant)\n"
